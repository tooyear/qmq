 ...
 * @since 2017/8/23
 */
public abstract class MessageLogVisitor<T> {
    private static final Logger LOG = LoggerFactory.getLogger(MessageLogVisitor.class);
    ...
    public MessageLogVisitor(final LogManager logManager, final long startOffset) {
    ...
    private Optional<T> readOneRecord(final ByteBuffer buffer) {
    ...
    protected abstract T createRecord(String subject, long sequence, long wroteOffset, int wroteBytes, short headerSize, long baseOffset, ByteBuffer currentBuffer);
}
